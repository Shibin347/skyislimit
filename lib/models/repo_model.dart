...
List<RepoModel> dataModelFromJson(String str) => List<RepoModel>.from(json.decode(str).map((x) => RepoModel.fromJson(x)));

String dataModelToJson(List<RepoModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
enum DefaultBranch { GH_PAGES, MASTER }
...
enum EventsUrl { HTTPS_API_GITHUB_COM_USERS_MOJOMBO_EVENTS_PRIVACY }

final eventsUrlValues = EnumValues({
  "https://api.github.com/users/mojombo/events{/privacy}": EventsUrl.HTTPS_API_GITHUB_COM_USERS_MOJOMBO_EVENTS_PRIVACY
});

enum FollowingUrl { HTTPS_API_GITHUB_COM_USERS_MOJOMBO_FOLLOWING_OTHER_USER }

final followingUrlValues = EnumValues({
  "https://api.github.com/users/mojombo/following{/other_user}": FollowingUrl.HTTPS_API_GITHUB_COM_USERS_MOJOMBO_FOLLOWING_OTHER_USER
});

enum GistsUrl { HTTPS_API_GITHUB_COM_USERS_MOJOMBO_GISTS_GIST_ID }

final gistsUrlValues = EnumValues({
  "https://api.github.com/users/mojombo/gists{/gist_id}": GistsUrl.HTTPS_API_GITHUB_COM_USERS_MOJOMBO_GISTS_GIST_ID
});

enum Login { MOJOMBO }

final loginValues = EnumValues({
  "mojombo": Login.MOJOMBO
});

enum OwnerNodeId { MDQ6_VX_NLCJ_E }

final ownerNodeIdValues = EnumValues({
  "MDQ6VXNlcjE=": OwnerNodeId.MDQ6_VX_NLCJ_E
});

enum StarredUrl { HTTPS_API_GITHUB_COM_USERS_MOJOMBO_STARRED_OWNER_REPO }

final starredUrlValues = EnumValues({
  "https://api.github.com/users/mojombo/starred{/owner}{/repo}": StarredUrl.HTTPS_API_GITHUB_COM_USERS_MOJOMBO_STARRED_OWNER_REPO
});
